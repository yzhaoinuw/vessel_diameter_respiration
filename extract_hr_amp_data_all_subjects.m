%% Extract compact HR and vessel amplitude data for all subjects
% Saves one hr_amp_data.mat file in each subject folder.

clear;

data_folder = fullfile(pwd, "data");
summary_file = fullfile(pwd, "results", "hr_amp_extraction_summary.csv");
...
for s = 1:numel(subjects)
    ...
    try
        result = extract_one_subject(subject_name, subject_folder, ...
            conf_thresh, start_min, bin_minutes, Fc_svm, ...
...
function result = extract_one_subject(subject_name, subject_folder, ...
    conf_thresh, start_min, bin_minutes, Fc_svm, hr_valid_bpm, ...
    min_beats_per_bin, hr_filter_bin_minutes, ...
    low_conf_rpeak_fraction_threshold, min_filtered_hr_samples_per_bin)
    ...
    if ~any(~is_dubious_hr_filtered_bin)
