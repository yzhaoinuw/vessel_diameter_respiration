%% Plot overlap Spearman HR-vessel scatter diagnostics and Wilcoxon summaries
...
if ~exist(by_mouse_csv, 'file') || ~exist(wilcoxon_csv, 'file')
    error('Missing overlap Spearman outputs. Run compute_hr_diam_corr_spearman_overlap.m first.');
end
...
for i = 1:numel(subjects)
    ...
    title_handle = sgtitle(sprintf('%s: overlap Spearman HR-vessel scatter diagnostics', subject_name), ...
        'Interpreter', 'none');
    title_handle.Color = 'k';

    out_file = fullfile(scatter_folder, subject_name + "_spearman_overlap_scatter.png");
...
title_handle = sgtitle('Across-mouse overlap Spearman correlations with Wilcoxon summary', ...
    'Interpreter', 'none', 'FontSize', 12);
title_handle.Color = 'k';

out_file = fullfile(summary_folder, 'spearman_overlap_wilcoxon_summary.png');
exportgraphics(fig, out_file, 'Resolution', 220);
close(fig);
fprintf('Saved %s\n', out_file);
