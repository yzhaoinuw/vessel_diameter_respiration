%% Compute Yue ECG-HR vs vessel-HR correlations across mouse subjects
% Uses avg_HR_Yue_10min from results.mat and compares it against
% results.hr (vessel cardiac peak frequency, converted to BPM).
...
for i = 1:numel(subjects)
    ...
    n_common = min([numel(timepts_min), numel(yue_hr_bpm), numel(vessel_hr_bpm)]);
    ...
    for w = 1:size(window_defs, 1)
        ...
        [r_spear, p_spear, n_spear] = corr_complete( ...
            yue_hr_bpm(mask), vessel_hr_bpm(mask), "Spearman", ...
            min_points_for_corr);
        [r_pear, p_pear, n_pear] = corr_complete( ...
            yue_hr_bpm(mask), vessel_hr_bpm(mask), "Pearson", ...
            min_points_for_corr);
...
by_mouse_csv = fullfile(output_folder, "hr_hr_corr_yue_vs_vessel_by_mouse.csv");
ttest_csv = fullfile(output_folder, "hr_hr_corr_yue_vs_vessel_ttest_summary.csv");
excluded_csv = fullfile(output_folder, "hr_hr_corr_yue_vs_vessel_excluded_subjects.csv");
excel_file = fullfile(output_folder, "hr_hr_corr_yue_vs_vessel_results.xlsx");
...
function plot_ttest_summary(by_mouse_table, ttest_table, fig_folder)
    ...
    sgtitle("avg_HR_Yue_10min vs vessel HR correlations with Fisher z t-test summary", ...
        "Interpreter", "none", "FontSize", 12);

    out_file = fullfile(fig_folder, "hr_hr_corr_yue_vs_vessel_ttest_summary.png");
