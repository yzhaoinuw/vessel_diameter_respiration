%% Compare rebinned instantaneous ECG HR against legacy avg_HR_Yue_10min
% Rebins beat-to-beat ECG HR from hr_amp_data.mat onto the legacy
% 10-minute windows used in results.mat, then computes per-mouse Pearson
% and Spearman correlations against results.avg_HR_Yue_10min. The script
% also saves per-mouse overlay plots of the two HR traces.
...
timeseries_folder = fullfile(fig_folder, "timeseries_by_mouse");
...
for i = 1:numel(subjects)
    ...
    hr_data = load(hr_amp_file, ...
        "t_hr", "hr_inst_bpm", "valid_inst_hr", "dubious_hr_bin_fraction");
    results_data = load(results_file, "timepts", "avg_HR_Yue_10min");

    dubious_fraction = hr_data.dubious_hr_bin_fraction;
    ...
    our_hr_bpm = rebin_ecg_hr_to_legacy_bins( ...
        double(hr_data.t_hr(:)), double(hr_data.hr_inst_bpm(:)), ...
        logical(hr_data.valid_inst_hr(:)), timepts_min, legacy_bin_minutes);

    n_common = min([numel(timepts_min), numel(yue_hr_bpm), numel(our_hr_bpm)]);
    timepts_min = timepts_min(1:n_common);
    yue_hr_bpm = yue_hr_bpm(1:n_common);
    our_hr_bpm = our_hr_bpm(1:n_common);

    row = table(subject_name, dubious_fraction, ...
        'VariableNames', {'Subject', 'DubiousHrBinFraction'});

    valid_overall = isfinite(our_hr_bpm) & isfinite(yue_hr_bpm);
    row.N_Overlap = nnz(valid_overall);
    row.MeanDiffBPM = mean(our_hr_bpm(valid_overall) - yue_hr_bpm(valid_overall), "omitnan");
    row.MeanAbsDiffBPM = mean(abs(our_hr_bpm(valid_overall) - yue_hr_bpm(valid_overall)), "omitnan");
    row.RMSE_BPM = sqrt(mean((our_hr_bpm(valid_overall) - yue_hr_bpm(valid_overall)).^2, "omitnan"));
    ...
    for w = 1:size(window_defs, 1)
        ...
        [r_spear, p_spear, n_spear] = corr_complete( ...
            our_hr_bpm(mask), yue_hr_bpm(mask), "Spearman", min_points_for_corr);
        [r_pear, p_pear, n_pear] = corr_complete( ...
            our_hr_bpm(mask), yue_hr_bpm(mask), "Pearson", min_points_for_corr);
    ...
    plot_mouse_overlay(subject_name, timepts_min, our_hr_bpm, yue_hr_bpm, timeseries_folder, row);
end

ttest_table = build_ttest_table(by_mouse_table);

by_mouse_csv = fullfile(output_folder, "hr_agreement_yue_by_mouse.csv");
ttest_csv = fullfile(output_folder, "hr_agreement_yue_ttest_summary.csv");
excluded_csv = fullfile(output_folder, "hr_agreement_yue_excluded_subjects.csv");
excel_file = fullfile(output_folder, "hr_agreement_yue_results.xlsx");
...
%% Local functions
function hr_binned = rebin_ecg_hr_to_legacy_bins(t_hr, hr_inst_bpm, valid_inst_hr, ...
    timepts_min, bin_minutes)
    ...
    for c = 1:numel(corr_types)
        ...
        for w = 1:numel(windows)
            ...
            ttest_table = [ttest_table; table(corr_type, window_name, ...
                n_subjects, ...
                mean(r_values, "omitnan"), ...
                median(r_values, "omitnan"), ...
                mean_z, tanh(mean_z), tanh(ci_low_z), tanh(ci_high_z), ...
                p_ttest, t_stat, df, ...
...
function plot_ttest_summary(by_mouse_table, ttest_table, fig_folder)
    ...
    sgtitle("Rebinned ECG HR vs avg_HR_Yue_10min Fisher z t-test summary", ...
        "Interpreter", "none", "FontSize", 12);

    out_file = fullfile(fig_folder, "hr_agreement_yue_ttest_summary.png");
    exportgraphics(fig, out_file, "Resolution", 220);
    close(fig);
    fprintf("Saved %s\n", out_file);
end

function plot_mouse_overlay(subject_name, timepts_min, our_hr_bpm, yue_hr_bpm, out_folder, row)
    fig = figure("Color", "w", "Position", [100 100 900 620], ...
        "Visible", "off");
    tiledlayout(2, 1, "TileSpacing", "compact", "Padding", "compact");

    nexttile;
    plot(timepts_min, our_hr_bpm, "-o", "Color", [0.10 0.45 0.80], ...
        "LineWidth", 1.5, "MarkerFaceColor", [0.10 0.45 0.80], ...
        "DisplayName", "Rebinned beat-to-beat HR");
    hold on;
    ...
    nexttile;
    diff_bpm = our_hr_bpm - yue_hr_bpm;
    ...
    out_file = fullfile(out_folder, subject_name + "_hr_agreement.png");
    exportgraphics(fig, out_file, "Resolution", 220);
    close(fig);
    fprintf("Saved %s\n", out_file);
end
