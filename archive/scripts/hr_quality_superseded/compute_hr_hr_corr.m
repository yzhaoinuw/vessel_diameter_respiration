%% Compute ECG-HR vs vessel-HR correlations across mouse subjects
...
% and Spearman correlations against results.hr (vessel cardiac peak
% frequency). Group-level inference uses one-sample t-tests on Fisher-z
% transformed mouse-level correlation coefficients.
...
for i = 1:numel(subjects)
    ...
    ecg_hr_bpm = rebin_ecg_hr_to_legacy_bins( ...
    ...
    for w = 1:size(window_defs, 1)
        ...
        [r_spear, p_spear, n_spear] = corr_complete( ...
            ecg_hr_bpm(mask), vessel_hr_bpm(mask), "Spearman", ...
            min_points_for_corr);
        [r_pear, p_pear, n_pear] = corr_complete( ...
            ecg_hr_bpm(mask), vessel_hr_bpm(mask), "Pearson", ...
            min_points_for_corr);
...
by_mouse_csv = fullfile(output_folder, "hr_hr_corr_by_mouse.csv");
ttest_csv = fullfile(output_folder, "hr_hr_corr_ttest_summary.csv");
excluded_csv = fullfile(output_folder, "hr_hr_corr_excluded_subjects.csv");
excel_file = fullfile(output_folder, "hr_hr_corr_results.xlsx");
...
function plot_ttest_summary(by_mouse_table, ttest_table, fig_folder)
    ...
    sgtitle("ECG HR vs vessel HR correlations with Fisher z t-test summary", ...
        "Interpreter", "none", "FontSize", 12);

    out_file = fullfile(fig_folder, "hr_hr_corr_ttest_summary.png");
