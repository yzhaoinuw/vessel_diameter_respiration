%% Run Fisher z one-sample t-tests on mouse-level Spearman correlations
...
excel_file = fullfile(output_folder, 'hr_amp_corr_spearman_results.xlsx');
ttest_csv = fullfile(output_folder, 'hr_amp_corr_spearman_ttest_summary.csv');

if ~exist(by_mouse_csv, 'file')
    error('Missing %s. Run compute_hr_diam_corr_spearman.m first.', by_mouse_csv);
end
