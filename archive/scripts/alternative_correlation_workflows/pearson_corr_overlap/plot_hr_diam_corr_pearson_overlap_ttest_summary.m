%% Plot Fisher z t-test summaries for overlap Pearson correlations
...
ttest_csv = fullfile(output_folder, 'hr_amp_corr_overlap_pearson_ttest_summary.csv');

if ~exist(by_mouse_csv, 'file') || ~exist(ttest_csv, 'file')
    error('Missing overlap Pearson t-test inputs. Run compute_hr_diam_corr_pearson_overlap_ttest.m first.');
end
...
title_handle = sgtitle('Across-mouse overlap Pearson correlations with Fisher z t-test summary', ...
    'Interpreter', 'none', 'FontSize', 12);
title_handle.Color = 'k';

out_file = fullfile(fig_folder, 'pearson_overlap_ttest_summary.png');
