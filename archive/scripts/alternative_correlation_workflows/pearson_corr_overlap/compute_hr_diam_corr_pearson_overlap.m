%% Compute Pearson correlations for filtered 1-minute HR vs overlapping vessel features
...
excel_file = fullfile(output_folder, 'hr_amp_corr_overlap_pearson_results.xlsx');
...
excluded_csv = fullfile(output_folder, 'hr_amp_corr_overlap_pearson_excluded_subjects.csv');
...
for i = 1:length(subjects)
    ...
    if hr_nan_fraction > hr_nan_exclusion_threshold
        ...
        excluded_table = [excluded_table; table(subject_name, hr_nan_fraction, ...
            ...
            'NFiniteAlignedHrBins', 'NAlignedBins'})];
        continue
    end
...
fprintf("\nSaved overlap Pearson outputs to:\n%s\n", output_folder);
fprintf("- %s\n", excel_file);
fprintf("- %s\n", by_mouse_csv);
fprintf("- %s\n", wilcoxon_csv);
fprintf("- %s\n", excluded_csv);
