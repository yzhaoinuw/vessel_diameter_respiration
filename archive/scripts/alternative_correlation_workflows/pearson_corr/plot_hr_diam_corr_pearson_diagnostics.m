%% Plot Pearson HR-vessel scatter diagnostics and Wilcoxon summaries
...
wilcoxon_csv = fullfile(output_folder, 'hr_amp_corr_pearson_wilcoxon_summary.csv');

if ~exist(by_mouse_csv, 'file') || ~exist(wilcoxon_csv, 'file')
    error('Missing Pearson correlation outputs. Run compute_hr_diam_corr_pearson.m first.');
end
...
for i = 1:length(subjects)
    ...
    sgtitle(sprintf('%s: Pearson HR-vessel scatter diagnostics', subject_name), ...
        'Interpreter', 'none');

    out_file = fullfile(scatter_folder, subject_name + "_pearson_scatter.png");
...
sgtitle('Across-mouse Pearson correlations with Wilcoxon summary', ...
    'Interpreter', 'none', 'FontSize', 12);

out_file = fullfile(summary_folder, 'pearson_wilcoxon_summary.png');
exportgraphics(fig, out_file, 'Resolution', 220);
close(fig);
fprintf('Saved %s\n', out_file);
