%% Plot Fisher z t-test summaries for Yue Spearman correlations

project_root = fileparts(fileparts(mfilename('fullpath')));
output_folder = fullfile(project_root, 'spearman_correlation_yue');
fig_folder = fullfile(output_folder, 'figures', 'ttest_summary');
if ~exist(fig_folder, 'dir'), mkdir(fig_folder); end

by_mouse_csv = fullfile(output_folder, 'hr_amp_corr_yue_spearman_by_mouse.csv');
ttest_csv = fullfile(output_folder, 'hr_amp_corr_yue_spearman_ttest_summary.csv');
if ~exist(by_mouse_csv, 'file') || ~exist(ttest_csv, 'file')
    error('Missing Yue Spearman t-test inputs. Run compute_hr_diam_corr_spearman_yue_ttest.m first.');
end
...
for v = 1:numel(vessel_vars)
    ...
    for w = 1:numel(window_names)
        ...
        subplot(numel(vessel_vars), numel(window_names), subplot_idx); hold on;
        ...
        summary_row = ttest_table.VesselVariable == vessel_var & ttest_table.Window == window_name;
        if any(summary_row)
            ...
            plot([0.5, numel(r_values) + 0.5], [mean_r mean_r], '-', 'Color', [0.85 0.20 0.20], 'LineWidth', 1.5);
            plot([0.5, numel(r_values) + 0.5], [ci_low ci_low], ':', 'Color', [0.85 0.20 0.20], 'LineWidth', 1.2);
            plot([0.5, numel(r_values) + 0.5], [ci_high ci_high], ':', 'Color', [0.85 0.20 0.20], 'LineWidth', 1.2);
            title_handle = title(sprintf('%s | %s\nT-test p=%.3g, mean rho=%.3f', ...
                vessel_label(vessel_var), window_name, ttest_table.P_TTest(summary_row), mean_r), ...
...
sgtitle('Across-mouse Spearman Yue correlations with Fisher z t-test summary', 'Interpreter', 'none', 'FontSize', 12);
out_file = fullfile(fig_folder, 'spearman_yue_ttest_summary.png');
